...
public class IdopontTorolAdapter extends RecyclerView.Adapter<IdopontTorolAdapter.ViewHolder> implements Filterable {
    private ArrayList<Idopont> mIdopontData;
    private ArrayList<Idopont> mIdopontDataAll;
    private Context mContext;
    private int lastPosition = -1;

    IdopontTorolAdapter(Context context, ArrayList<Idopont> itemsData){
    ...
    @Override
    public void onBindViewHolder(IdopontTorolAdapter.ViewHolder holder, int position) {
        Idopont currentIdopont = mIdopontData.get(position);
        holder.bindTo(currentIdopont);

        if(holder.getAdapterPosition() > lastPosition){
            Animation animation = AnimationUtils.loadAnimation(mContext, R.anim.anim_2);
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            itemView.findViewById(R.id.deleteButton).setOnClickListener(view -> ((SzakemberIdopontTorolActivity)mContext).torol(
